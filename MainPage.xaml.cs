        ...
        foreach (var hormone in KnowledgeBase.Hormones)
        {
            ...
            if (hormone.Id == "h_001")
            {
                ...
                if (activity == "High")
                {
                    ...
                    hormone.Description += " [АТЛЕТ: Целевой уровень андрогенов повышен для компенсации тренировочного стресса]";
                }
            ...
            if (hormone.Id == "h_004")
            {
                ...
                if (hasStress)
                {
                    ...
                    hormone.Description += " [КОРРЕКЦИЯ: Верхняя граница расширена с учетом хронической стресс-нагрузки]";
                }
            }
        }
    }
